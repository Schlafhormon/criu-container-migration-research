...
POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS="${POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS:-4800}"
POSTCOPY_EARLY_READINESS="${POSTCOPY_EARLY_READINESS:-1}"
POSTCOPY_EARLY_READINESS_PROBE_MAX_TIME_S="${POSTCOPY_EARLY_READINESS_PROBE_MAX_TIME_S:-0.25}"
...
LAZY_PID_FILE=""
CKPT_PID_FILE=""
CHECKPOINT_DONE_EMITTED=0

wait_for_checkpoint_done(){
  local rounds="${1:-50}"
  local interval_s="${2:-0.1}"
  local pid _
  if [ "$CHECKPOINT_DONE_EMITTED" = "1" ]; then
    return 0
  fi
  if [ -z "$CKPT_PID_FILE" ] || [ ! -f "$CKPT_PID_FILE" ]; then
    emit_event checkpoint_wait_skipped reason=no_pid_file
    return 1
  fi
  pid="$(cat "$CKPT_PID_FILE" 2>/dev/null || true)"
  if [ -z "$pid" ]; then
    emit_event checkpoint_wait_skipped reason=empty_pid_file
    return 1
  fi
  for _ in $(seq 1 "$rounds"); do
    if ! ps -p "$pid" >/dev/null 2>&1; then
      t_dump_end=$(ms)
      CHECKPOINT_DONE_EMITTED=1
  ...
  emit_event checkpoint_wait_timeout pid="$pid" rounds="$rounds" interval_s="$interval_s"
  return 1
}
...
emit_event vip_cutover_config \
  ...
  early_readiness="$POSTCOPY_EARLY_READINESS" \
  early_readiness_probe_max_time_s="$POSTCOPY_EARLY_READINESS_PROBE_MAX_TIME_S" \
  remote_events="$POSTCOPY_REMOTE_EVENTS"
...
readiness_async_pid=""
readiness_async_status_file=""
if [ "$(bool_to_int "$POSTCOPY_EARLY_READINESS")" = "1" ]; then
  readiness_async_status_file="$(mktemp "/tmp/clm-postcopy-readiness.${RUN_ID}.XXXXXX")"
  emit_event dest_readiness_async_start probe_max_time_s="$POSTCOPY_EARLY_READINESS_PROBE_MAX_TIME_S" \
    status_file="$readiness_async_status_file"
  (
    set +e
    POSTCOPY_PROBE_MAX_TIME_S="$POSTCOPY_EARLY_READINESS_PROBE_MAX_TIME_S"
    wait_for_postcopy_readiness
    rc=$?
    printf '%s\n' "$rc" > "$readiness_async_status_file"
    exit "$rc"
  ) &
  readiness_async_pid=$!
else
  emit_event dest_readiness_async_skipped early_readiness="$POSTCOPY_EARLY_READINESS"
fi

wait_for_checkpoint_done 50 0.1 || true

if [ -n "$readiness_async_pid" ]; then
  readiness_async_rc=0
  if wait "$readiness_async_pid"; then
    readiness_async_rc=0
  else
    readiness_async_rc=$?
  fi
  emit_event dest_readiness_async_done rc="$readiness_async_rc" status_file="$readiness_async_status_file"
  [ "$readiness_async_rc" -eq 0 ] || fail "Dest-Readiness wurde vor VIP-Cutover nicht stabil."
else
...
echo "early_readiness=$POSTCOPY_EARLY_READINESS"
echo "early_readiness_probe_max_time_s=$POSTCOPY_EARLY_READINESS_PROBE_MAX_TIME_S"
