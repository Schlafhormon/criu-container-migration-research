...
RUNC_CP_FLAGS="${RUNC_CP_FLAGS:---manage-cgroups-mode soft --shell-job --leave-running}"
...
POSTCOPY_BUNDLE_TRANSFER_MODE="${POSTCOPY_BUNDLE_TRANSFER_MODE:-local_copy}"
RUNC_BUNDLE_DST_SHARED="${RUNC_BUNDLE_DST_SHARED:-${RUNC_BUNDLE_DST:-/mnt/criu/runc-bundle}}"
RUNC_BUNDLE_DST_LOCAL="${RUNC_BUNDLE_DST_LOCAL:-/tmp/criu-local-runc-bundle/$NAME/$CP_NAME}"
RUNC_BUNDLE_DST="${RUNC_BUNDLE_DST:-/mnt/criu/runc-bundle}"
if [ "$POSTCOPY_BUNDLE_TRANSFER_MODE" = "local_copy" ]; then
  RUNC_BUNDLE_DST="$RUNC_BUNDLE_DST_LOCAL"
fi
IMAGES_BASE_SRC="${IMAGES_BASE_SRC:-$SRC_NFS_ROOT/runc/$NAME/$CP_NAME}"
IMAGES_BASE_DST_WAS_SET="${IMAGES_BASE_DST+x}"
IMAGES_BASE_DST="${IMAGES_BASE_DST:-$DST_LOCAL_ROOT/runc/$NAME/$CP_NAME}"
POSTCOPY_IMAGE_TRANSFER_MODE="${POSTCOPY_IMAGE_TRANSFER_MODE:-local_copy}"
POSTCOPY_REMOTE_EVENTS="${POSTCOPY_REMOTE_EVENTS:-0}"
POSTCOPY_LAZY_DEBUG_CHECKS="${POSTCOPY_LAZY_DEBUG_CHECKS:-0}"
POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS="${POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS:-5200}"
POSTCOPY_COLLECT_DEST_ARTIFACTS="${POSTCOPY_COLLECT_DEST_ARTIFACTS:-1}"
POSTCOPY_RESTORE_WATCH_ENABLED="${POSTCOPY_RESTORE_WATCH_ENABLED:-0}"
POSTCOPY_RESTORE_WATCH_INTERVAL_MS="${POSTCOPY_RESTORE_WATCH_INTERVAL_MS:-200}"
POSTCOPY_RESTORE_WATCH_DURATION_MS="${POSTCOPY_RESTORE_WATCH_DURATION_MS:-9000}"
if [ "$POSTCOPY_IMAGE_TRANSFER_MODE" = "shared_nfs" ] && [ -z "$IMAGES_BASE_DST_WAS_SET" ]; then
  IMAGES_BASE_DST="${REMOTE_NFS_ROOT}/runc/$NAME/$CP_NAME"
fi
DST_WORK_DIR_WAS_SET="${DST_WORK_DIR+x}"
DST_WORK_DIR="${DST_WORK_DIR:-$IMAGES_BASE_DST/final/work}"
if [ "$POSTCOPY_IMAGE_TRANSFER_MODE" = "shared_nfs" ] && [ -z "${DST_WORK_DIR_WAS_SET:-}" ]; then
  DST_WORK_DIR="$DST_LOCAL_ROOT/runc/$NAME/$CP_NAME/final/work"
fi
DST_RUNTIME_DIR="${DST_RUNTIME_DIR:-$DST_LOCAL_ROOT/runc/$NAME/$CP_NAME}"
...
emit_event_remote(){
  ...
  [ "${POSTCOPY_REMOTE_EVENTS:-0}" = "1" ] || return 0
  local name="$1"; shift
  local ts="\$(date +%s%3N)"
...
ckpt_stop_async(){
  local reason="${1:-abort}" pid="" pgid="" image_dir="${final_dir:-}"

  emit_event checkpoint_stop_start reason="$reason" || true
  if [ -n "$CKPT_PID_FILE" ] && [ -f "$CKPT_PID_FILE" ]; then
    pid="$(cat "$CKPT_PID_FILE" 2>/dev/null || true)"
  ...
  if [ -n "$image_dir" ]; then
    pkill -TERM -f "runc .*checkpoint ${NAME} .*${image_dir}" 2>/dev/null || true
    pkill -TERM -f "sudo runc .*checkpoint ${NAME} .*${image_dir}" 2>/dev/null || true
  fi
  command -v fuser >/dev/null 2>&1 && fuser -TERM -k "${LAZY_PORT}/tcp" 2>/dev/null || true
  ...
  if [ -n "$image_dir" ]; then
    pkill -KILL -f "runc .*checkpoint ${NAME} .*${image_dir}" 2>/dev/null || true
    pkill -KILL -f "sudo runc .*checkpoint ${NAME} .*${image_dir}" 2>/dev/null || true
  fi
  command -v fuser >/dev/null 2>&1 && fuser -KILL -k "${LAZY_PORT}/tcp" 2>/dev/null || true
  emit_event checkpoint_stop_done reason="$reason" || true
}
...
collect_dest_criu_artifacts(){
  local reason="${1:-manual}" start_ms dest_artifact_dir_q dst_work_dir_q dst_runtime_dir_q lazy_log_q
  [ "$(bool_to_int "$POSTCOPY_COLLECT_DEST_ARTIFACTS")" = "1" ] || return 0
  start_ms="$(ms)"
  emit_event dest_artifacts_collect_start reason="$reason" dst_work_dir="$DST_WORK_DIR" dst_runtime_dir="$DST_RUNTIME_DIR"
  dest_artifact_dir="$IMAGES_BASE_SRC/dest-artifacts/$reason"
  dest_artifact_dir_q="$(shell_escape_sq "$dest_artifact_dir")"
  dst_work_dir_q="$(shell_escape_sq "$DST_WORK_DIR")"
  dst_runtime_dir_q="$(shell_escape_sq "$DST_RUNTIME_DIR")"
  lazy_log_q="$(shell_escape_sq "${LAZY_LOG:-}")"
  $SSH "sudo bash -lc 'set +e
mkdir -p \"$dest_artifact_dir_q\"
date +%s%3N > \"$dest_artifact_dir_q/collect_ts_ms.txt\"
hostname > \"$dest_artifact_dir_q/hostname.txt\" 2>&1
criu --version > \"$dest_artifact_dir_q/criu-version.txt\" 2>&1
runc --version > \"$dest_artifact_dir_q/runc-version.txt\" 2>&1
ps -eo pid,ppid,stat,wchan:32,cmd > \"$dest_artifact_dir_q/ps.txt\" 2>&1
ss -tanp > \"$dest_artifact_dir_q/ss-tanp.txt\" 2>&1
if [ -d \"$dst_work_dir_q\" ]; then
  find \"$dst_work_dir_q\" -maxdepth 2 -printf \"%TY-%Tm-%Td %TH:%TM:%TS %s %p\\n\" > \"$dest_artifact_dir_q/work-find.txt\" 2>&1
  cp -a \"$dst_work_dir_q\" \"$dest_artifact_dir_q/work\" 2> \"$dest_artifact_dir_q/work-copy.err\" || true
fi
if [ -d \"$dst_runtime_dir_q\" ]; then
  find \"$dst_runtime_dir_q\" -maxdepth 2 -printf \"%TY-%Tm-%Td %TH:%TM:%TS %s %p\\n\" > \"$dest_artifact_dir_q/runtime-find.txt\" 2>&1
fi
if [ -f \"$lazy_log_q\" ]; then
  cp -a \"$lazy_log_q\" \"$dest_artifact_dir_q/lazy-pages.log\" 2> \"$dest_artifact_dir_q/lazy-pages-copy.err\" || true
fi
if [ -f \"$dst_runtime_dir_q/lazy-pages.pid\" ]; then
  cp -a \"$dst_runtime_dir_q/lazy-pages.pid\" \"$dest_artifact_dir_q/lazy-pages.pid\" 2>/dev/null || true
fi
chmod -R a+rX \"$dest_artifact_dir_q\" 2>/dev/null || true
' " || true
  emit_event dest_artifacts_collect_done reason="$reason" dest="$dest_artifact_dir" dur_ms=$(( $(ms) - start_ms ))
}

start_restore_watch(){
  [ "$(bool_to_int "$POSTCOPY_RESTORE_WATCH_ENABLED")" = "1" ] || return 0
  local watch_dir watch_dir_q interval_ms duration_ms start_ms
  interval_ms="$(positive_int_or_default "$POSTCOPY_RESTORE_WATCH_INTERVAL_MS" 200)"
  duration_ms="$(positive_int_or_default "$POSTCOPY_RESTORE_WATCH_DURATION_MS" 9000)"
  watch_dir="$IMAGES_BASE_SRC/dest-artifacts/restore_watch"
  watch_dir_q="$(shell_escape_sq "$watch_dir")"
  start_ms="$(ms)"
  emit_event restore_watch_start dest="$watch_dir" interval_ms="$interval_ms" duration_ms="$duration_ms"
  $SSH "sudo bash -lc 'set +e
mkdir -p \"$watch_dir_q\"
(
  echo \"restore_watch_start_ms=\$(date +%s%3N)\"
  echo \"interval_ms=$interval_ms\"
  echo \"duration_ms=$duration_ms\"
  start_ms=\$(date +%s%3N)
  end_ms=\$((start_ms + $duration_ms))
  i=0
  while [ \"\$(date +%s%3N)\" -le \"\$end_ms\" ]; do
    ts_ms=\$(date +%s%3N)
    sample_dir=\"$watch_dir_q/sample_\${i}_\${ts_ms}\"
    mkdir -p \"\$sample_dir/proc\"
    printf \"%s\n\" \"\$ts_ms\" > \"\$sample_dir/ts_ms.txt\"
    ps -eLo pid,ppid,tid,stat,wchan:32,comm,args > \"\$sample_dir/ps-threads.txt\" 2>&1
    pgrep -af \"runc|criu|gunicorn\" > \"\$sample_dir/pgrep-runc-criu-gunicorn.txt\" 2>&1 || true
    ss -tanp > \"\$sample_dir/ss-tanp.txt\" 2>&1
    for pid in \$(pgrep -f \"runc|criu\" 2>/dev/null || true); do
      [ -d \"/proc/\$pid\" ] || continue
      pid_dir=\"\$sample_dir/proc/\$pid\"
      mkdir -p \"\$pid_dir/task\"
      cat \"/proc/\$pid/status\" > \"\$pid_dir/status.txt\" 2>&1 || true
      cat \"/proc/\$pid/wchan\" > \"\$pid_dir/wchan.txt\" 2>&1 || true
      cat \"/proc/\$pid/syscall\" > \"\$pid_dir/syscall.txt\" 2>&1 || true
      cat \"/proc/\$pid/stack\" > \"\$pid_dir/stack.txt\" 2>&1 || true
      for task in /proc/\$pid/task/*; do
        [ -d \"\$task\" ] || continue
        tid=\${task##*/}
        tid_dir=\"\$pid_dir/task/\$tid\"
        mkdir -p \"\$tid_dir\"
        cat \"\$task/status\" > \"\$tid_dir/status.txt\" 2>&1 || true
        cat \"\$task/wchan\" > \"\$tid_dir/wchan.txt\" 2>&1 || true
        cat \"\$task/syscall\" > \"\$tid_dir/syscall.txt\" 2>&1 || true
        cat \"\$task/stack\" > \"\$tid_dir/stack.txt\" 2>&1 || true
      done
    done
    i=\$((i + 1))
    sleep \$(awk -v ms=$interval_ms \"BEGIN { printf \\\"%.3f\\\", ms / 1000.0 }\")
  done
  echo \"restore_watch_done_ms=\$(date +%s%3N)\"
) > \"$watch_dir_q/restore-watch.log\" 2>&1 &
echo \$! > \"$watch_dir_q/restore-watch.pid\"
chmod -R a+rX \"$watch_dir_q\" 2>/dev/null || true
'" || true
  emit_event restore_watch_started dest="$watch_dir" dur_ms=$(( $(ms) - start_ms ))
}

stop_restore_watch(){
  [ "$(bool_to_int "$POSTCOPY_RESTORE_WATCH_ENABLED")" = "1" ] || return 0
  local watch_dir watch_dir_q start_ms
  watch_dir="$IMAGES_BASE_SRC/dest-artifacts/restore_watch"
  watch_dir_q="$(shell_escape_sq "$watch_dir")"
  start_ms="$(ms)"
  $SSH "sudo bash -lc 'set +e
if [ -f \"$watch_dir_q/restore-watch.pid\" ]; then
  pid=\$(cat \"$watch_dir_q/restore-watch.pid\" 2>/dev/null || true)
  if [ -n \"\$pid\" ]; then
...
date +%s%3N > \"$watch_dir_q/stop_ts_ms.txt\"
chmod -R a+rX \"$watch_dir_q\" 2>/dev/null || true
'" || true
  emit_event restore_watch_done dest="$watch_dir" dur_ms=$(( $(ms) - start_ms ))
}
...
cleanup(){
  ...
  if [ $rc -ne 0 ]; then
    collect_dest_criu_artifacts "cleanup_rc_${rc}" || true
  fi
  ...
  if [ $rc -ne 0 ] && [ -n "$CKPT_PID_FILE" ] && [ -f "$CKPT_PID_FILE" ]; then
    ckpt_stop_async "cleanup_rc_${rc}" || true
  fi
...
emit_event vip_cutover_config \
  ...
  src_forward_target_port="$POSTCOPY_SRC_FORWARD_TARGET_PORT" image_transfer_mode="$POSTCOPY_IMAGE_TRANSFER_MODE" \
  bundle_transfer_mode="$POSTCOPY_BUNDLE_TRANSFER_MODE" bundle_dst_shared="$RUNC_BUNDLE_DST_SHARED" \
  bundle_dst_local="$RUNC_BUNDLE_DST_LOCAL" bundle_dst="$RUNC_BUNDLE_DST" \
  lazy_debug_checks="$POSTCOPY_LAZY_DEBUG_CHECKS" \
  restore_min_after_checkpoint_ms="$POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS" remote_events="$POSTCOPY_REMOTE_EVENTS" \
  restore_watch_enabled="$POSTCOPY_RESTORE_WATCH_ENABLED" restore_watch_interval_ms="$POSTCOPY_RESTORE_WATCH_INTERVAL_MS" \
  restore_watch_duration_ms="$POSTCOPY_RESTORE_WATCH_DURATION_MS"
...
$SSH "command -v tar >/dev/null" || fail "tar fehlt Ziel"
case "$POSTCOPY_BUNDLE_TRANSFER_MODE" in
  shared_nfs)
    $SSH "[ -d '$RUNC_BUNDLE_DST' ]" || fail "runc-Bundle fehlt Ziel: $RUNC_BUNDLE_DST"
    ;;
  local_copy)
    $SSH "[ -d '$RUNC_BUNDLE_DST_SHARED' ]" || fail "runc-Bundle-Quelle fehlt Ziel: $RUNC_BUNDLE_DST_SHARED"
    ;;
  *)
    fail "Unbekannter POSTCOPY_BUNDLE_TRANSFER_MODE='$POSTCOPY_BUNDLE_TRANSFER_MODE' (erlaubt: shared_nfs, local_copy)"
    ;;
esac
if [ "$POSTCOPY_IMAGE_TRANSFER_MODE" = "shared_nfs" ] || [ "$POSTCOPY_BUNDLE_TRANSFER_MODE" = "local_copy" ]; then
  $SSH "mount | grep -q '$REMOTE_NFS_ROOT'" || fail "NFS $REMOTE_NFS_ROOT ist auf dem Ziel nicht gemountet"
fi
if [ "$POSTCOPY_IMAGE_TRANSFER_MODE" = "shared_nfs" ]; then
  $SSH "sudo mkdir -p '$DST_LOCAL_ROOT' '$DST_WORK_DIR' '$DST_RUNTIME_DIR'"
else
  $SSH "sudo mkdir -p '$DST_LOCAL_ROOT' '$IMAGES_BASE_DST' '$DST_WORK_DIR' '$DST_RUNTIME_DIR'"
fi

case "$POSTCOPY_BUNDLE_TRANSFER_MODE" in
  shared_nfs)
    log "Nutze runc-Bundle auf Ziel direkt: $RUNC_BUNDLE_DST"
    ;;
  local_copy)
    log "Spiegele runc-Bundle/rootfs lokal auf Ziel via tar: $RUNC_BUNDLE_DST"
    [ "$RUNC_BUNDLE_DST" != "$RUNC_BUNDLE_DST_SHARED" ] || fail "Bundle local_copy wuerde Quelle ueberschreiben: $RUNC_BUNDLE_DST"
    bundle_prepare_start_ms="$(ms)"
    emit_event bundle_prepare_start mode=local_copy method=tar source="$RUNC_BUNDLE_DST_SHARED" dest="$RUNC_BUNDLE_DST"
    run "$SSH \"sudo rm -rf '$RUNC_BUNDLE_DST' && sudo mkdir -p '$RUNC_BUNDLE_DST' && cd '$RUNC_BUNDLE_DST_SHARED' && sudo tar -cf - . | sudo tar --no-same-owner -xpf - -C '$RUNC_BUNDLE_DST'\""
    emit_event bundle_prepare_done mode=local_copy method=tar dur_ms=$(( $(ms) - bundle_prepare_start_ms ))
    ;;
esac
...
log "Bereite Images auf Ziel vor (mode=$POSTCOPY_IMAGE_TRANSFER_MODE)…"
t_tx_start=$(ms)
...
case "$POSTCOPY_IMAGE_TRANSFER_MODE" in
  shared_nfs)
    log "Nutze geteilten NFS-Image-Pfad direkt: $IMAGES_BASE_DST"
    run "$SSH \"sudo mkdir -p '$DST_WORK_DIR' '$DST_RUNTIME_DIR'\""
    ;;
  local_copy)
    log "Spiegele Images nach benke2…"
    run "$SSH \"sudo rm -rf '$IMAGES_BASE_DST' '$DST_WORK_DIR' '$DST_RUNTIME_DIR' && sudo mkdir -p '$IMAGES_BASE_DST' '$DST_WORK_DIR' '$DST_RUNTIME_DIR'\""
    run "$SSH \"sudo cp -a --no-preserve=mode,ownership,timestamps '${REMOTE_NFS_ROOT}/runc/$NAME/$CP_NAME/.' '$IMAGES_BASE_DST/'\""
    ;;
  *)
    fail "Unbekannter POSTCOPY_IMAGE_TRANSFER_MODE='$POSTCOPY_IMAGE_TRANSFER_MODE' (erlaubt: shared_nfs, local_copy)"
    ;;
esac
...
log "Image-Vorbereitung abgeschlossen in $((t_tx_end - t_tx_start)) ms"
...
nat_clear_dst || true
src_forward_clear || true
emit_event vip_prepare_done dur_ms=$(( $(ms) - vip_prepare_start_ms ))

emit_event_remote lazy_daemon_start addr="$SRC_LAZY_IP" port="$LAZY_PORT"
LAZY_LOG="$DST_RUNTIME_DIR/lazy-pages.log"; LAZY_PID_FILE="$DST_RUNTIME_DIR/lazy-pages.pid"
...
run "$SSH \"sudo bash -c 'nohup criu lazy-pages \
  ...
  --work-dir  \\\"$DST_WORK_DIR\\\" \
  --page-server --address $SRC_LAZY_IP --port $LAZY_PORT \
  --lazy-pages >\\\"$LAZY_LOG\\\" 2>&1 & echo \\$! >\\\"$LAZY_PID_FILE\\\"'\""
emit_event lazy_prepare_done dur_ms=$(( $(ms) - lazy_prepare_start_ms )) log="$LAZY_LOG" pid_file="$LAZY_PID_FILE"

if [ "$(bool_to_int "$POSTCOPY_LAZY_DEBUG_CHECKS")" = "1" ]; then
  emit_event lazy_debug_checks_start
  lazy_debug_start_ms="$(ms)"
  run "$SSH \"pgrep -a criu | grep lazy-pages || true\""
  run "$SSH \"test -S '$DST_WORK_DIR/lazy-pages.socket' || (echo 'WARN: lazy-pages.socket fehlt (noch)' >&2)\""
  emit_event lazy_debug_checks_done dur_ms=$(( $(ms) - lazy_debug_start_ms ))
else
  emit_event lazy_debug_checks_skipped
fi

restore_min_after_checkpoint_ms="$(nonnegative_int_or_default "$POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS" 4800)"
if [ "$restore_min_after_checkpoint_ms" -gt 0 ]; then
  restore_not_before_ms=$((t_dump_start + restore_min_after_checkpoint_ms))
  restore_gate_now_ms="$(ms)"
  if [ "$restore_gate_now_ms" -lt "$restore_not_before_ms" ]; then
    restore_gate_wait_ms=$((restore_not_before_ms - restore_gate_now_ms))
    restore_gate_start_ms="$restore_gate_now_ms"
    emit_event restore_min_wait_start wait_ms="$restore_gate_wait_ms" \
      min_after_checkpoint_ms="$restore_min_after_checkpoint_ms" checkpoint_start_ms="$t_dump_start"
    sleep "$(ms_to_sleep_s "$restore_gate_wait_ms")"
    emit_event restore_min_wait_done dur_ms=$(( $(ms) - restore_gate_start_ms ))
  else
    emit_event restore_min_wait_skipped wait_ms=0 min_after_checkpoint_ms="$restore_min_after_checkpoint_ms"
  fi
else
  emit_event restore_min_wait_skipped wait_ms=0 min_after_checkpoint_ms="$restore_min_after_checkpoint_ms"
fi
...
restore_rc=$?
set -e
stop_restore_watch || true
[ "$restore_rc" -eq 0 ] || exit "$restore_rc"

t_restore_end=$(ms)
emit_event restore_cmd_done dur_ms=$((t_restore_end - t_restore_start))
...
collect_dest_criu_artifacts "after_restore" || true
...
if [ -n "$CKPT_PID_FILE" ] && [ -f "$CKPT_PID_FILE" ]; then
  for i in $(seq 1 50); do
    if ! ps -p "$(cat "$CKPT_PID_FILE")" >/dev/null 2>&1; then
      ...
      collect_dest_criu_artifacts "after_checkpoint_done" || true
      break
    fi
    sleep 0.1
  done
...
collect_dest_criu_artifacts "before_lazy_stop" || true
...
emit_event summary mode=$MODE name=$NAME cp=$CP_NAME tcp_est=$TCP_EST lazy_port=$LAZY_PORT lazy_addr="$SRC_LAZY_ADDR" \
  ...
  image_transfer_mode="$POSTCOPY_IMAGE_TRANSFER_MODE" dst_work_dir="$DST_WORK_DIR" \
  bundle_transfer_mode="$POSTCOPY_BUNDLE_TRANSFER_MODE" bundle_dst_shared="$RUNC_BUNDLE_DST_SHARED" \
  bundle_dst_local="$RUNC_BUNDLE_DST_LOCAL" \
  lazy_debug_checks="$POSTCOPY_LAZY_DEBUG_CHECKS" \
  restore_min_after_checkpoint_ms="$POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS" \
  collect_dest_artifacts="$POSTCOPY_COLLECT_DEST_ARTIFACTS" remote_events="$POSTCOPY_REMOTE_EVENTS" \
  restore_watch_enabled="$POSTCOPY_RESTORE_WATCH_ENABLED" restore_watch_interval_ms="$POSTCOPY_RESTORE_WATCH_INTERVAL_MS" \
  restore_watch_duration_ms="$POSTCOPY_RESTORE_WATCH_DURATION_MS"
...
echo "dst_work_dir=$DST_WORK_DIR"
echo "image_transfer_mode=$POSTCOPY_IMAGE_TRANSFER_MODE"
echo "bundle_transfer_mode=$POSTCOPY_BUNDLE_TRANSFER_MODE"
echo "bundle_dst_shared=$RUNC_BUNDLE_DST_SHARED"
echo "bundle_dst_local=$RUNC_BUNDLE_DST_LOCAL"
echo "lazy_debug_checks=$POSTCOPY_LAZY_DEBUG_CHECKS"
echo "restore_min_after_checkpoint_ms=$POSTCOPY_RESTORE_MIN_AFTER_CHECKPOINT_MS"
echo "collect_dest_artifacts=$POSTCOPY_COLLECT_DEST_ARTIFACTS"
echo "remote_events=$POSTCOPY_REMOTE_EVENTS"
echo "restore_watch_enabled=$POSTCOPY_RESTORE_WATCH_ENABLED"
echo "restore_watch_interval_ms=$POSTCOPY_RESTORE_WATCH_INTERVAL_MS"
echo "restore_watch_duration_ms=$POSTCOPY_RESTORE_WATCH_DURATION_MS"
