...
# Conservative minimal-path variant of
...
# directly comparable with the original transfer method.
...
emit_event script_start mode="$MODE" name="$NAME" run_id="$RUN_ID" variant=minimal transfer_mode=destination_nfs_copy
...
emit_event summary mode="$MODE" name="$NAME" cp="$CP_NAME" variant=minimal transfer_mode=destination_nfs_copy images_src="$IMAGES_BASE_SRC" images_dst="$IMAGES_BASE_DST" bundle_dst="$RUNC_BUNDLE_DST" vip="$VIP_ADDR" net_mode="$NET_MODE"
...
log "Minimale Post-Copy-Migration + VIP Cutover abgeschlossen."
