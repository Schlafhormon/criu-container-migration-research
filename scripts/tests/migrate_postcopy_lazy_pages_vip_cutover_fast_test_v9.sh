...
POSTCOPY_LAZY_SOCKET_WAIT_MS="${POSTCOPY_LAZY_SOCKET_WAIT_MS:-1500}"
POSTCOPY_LAZY_SOCKET_WAIT_INTERVAL_MS="${POSTCOPY_LAZY_SOCKET_WAIT_INTERVAL_MS:-50}"
...
emit_event vip_cutover_config \
  ...
  lazy_debug_checks="$POSTCOPY_LAZY_DEBUG_CHECKS" lazy_socket_wait_ms="$POSTCOPY_LAZY_SOCKET_WAIT_MS" \
  lazy_socket_wait_interval_ms="$POSTCOPY_LAZY_SOCKET_WAIT_INTERVAL_MS" remote_events="$POSTCOPY_REMOTE_EVENTS"
...
lazy_socket_wait_ms="$(nonnegative_int_or_default "$POSTCOPY_LAZY_SOCKET_WAIT_MS" 1500)"
if [ "$lazy_socket_wait_ms" -gt 0 ]; then
  lazy_socket_interval_ms="$(positive_int_or_default "$POSTCOPY_LAZY_SOCKET_WAIT_INTERVAL_MS" 50)"
  lazy_socket_interval_s="$(ms_to_sleep_s "$lazy_socket_interval_ms")"
  lazy_socket_attempts=$(( (lazy_socket_wait_ms + lazy_socket_interval_ms - 1) / lazy_socket_interval_ms ))
  [ "$lazy_socket_attempts" -gt 0 ] || lazy_socket_attempts=1
  lazy_socket_start_ms="$(ms)"
  emit_event lazy_socket_wait_start socket="$DST_WORK_DIR/lazy-pages.socket" \
    timeout_ms="$lazy_socket_wait_ms" interval_ms="$lazy_socket_interval_ms" attempts="$lazy_socket_attempts"
  lazy_socket_path="$DST_WORK_DIR/lazy-pages.socket"
  lazy_socket_path_q="$(shell_escape_sq "$lazy_socket_path")"
  lazy_socket_remote_script="i=1; while [ \"\$i\" -le $lazy_socket_attempts ]; do test -S '$lazy_socket_path_q' && exit 0; sleep $lazy_socket_interval_s; i=\$((i + 1)); done; test -S '$lazy_socket_path_q'"
  lazy_socket_remote_script_q="$(shell_escape_sq "$lazy_socket_remote_script")"
  if [ "$DRY_RUN" = "1" ]; then
    echo "DRY: $SSH \"bash -lc '$lazy_socket_remote_script_q'\""
    emit_event lazy_socket_wait_done ok=1 dur_ms=$(( $(ms) - lazy_socket_start_ms )) dry_run=1
  elif $SSH "bash -lc '$lazy_socket_remote_script_q'"; then
    emit_event lazy_socket_wait_done ok=1 dur_ms=$(( $(ms) - lazy_socket_start_ms ))
  else
    emit_event lazy_socket_wait_done ok=0 dur_ms=$(( $(ms) - lazy_socket_start_ms ))
    fail "lazy-pages.socket wurde vor Restore nicht rechtzeitig bereit."
  fi
else
  emit_event lazy_socket_wait_skipped timeout_ms="$lazy_socket_wait_ms"
fi
...
echo "lazy_socket_wait_ms=$POSTCOPY_LAZY_SOCKET_WAIT_MS"
echo "lazy_socket_wait_interval_ms=$POSTCOPY_LAZY_SOCKET_WAIT_INTERVAL_MS"
