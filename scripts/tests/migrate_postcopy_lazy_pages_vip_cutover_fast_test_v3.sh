...
POSTCOPY_TRANSFER_VERIFY_MODE="${POSTCOPY_TRANSFER_VERIFY_MODE:-none}"
POSTCOPY_LAZY_DEBUG_CHECKS="${POSTCOPY_LAZY_DEBUG_CHECKS:-0}"
POSTCOPY_PREP_DST_BEFORE_CHECKPOINT="${POSTCOPY_PREP_DST_BEFORE_CHECKPOINT:-1}"
POSTCOPY_REMOTE_EVENTS="${POSTCOPY_REMOTE_EVENTS:-0}"
POSTCOPY_RESTORE_GATE_MIN_AFTER_CHECKPOINT_MS="${POSTCOPY_RESTORE_GATE_MIN_AFTER_CHECKPOINT_MS:-4500}"
...
emit_event vip_cutover_config \
  ...
  transfer_verify_mode="$POSTCOPY_TRANSFER_VERIFY_MODE" lazy_debug_checks="$POSTCOPY_LAZY_DEBUG_CHECKS" \
  prep_dst_before_checkpoint="$POSTCOPY_PREP_DST_BEFORE_CHECKPOINT" remote_events="$POSTCOPY_REMOTE_EVENTS" \
  restore_gate_min_after_checkpoint_ms="$POSTCOPY_RESTORE_GATE_MIN_AFTER_CHECKPOINT_MS"
...
if [ "$(bool_to_int "$POSTCOPY_PREP_DST_BEFORE_CHECKPOINT")" = "1" ]; then
  emit_event dst_preprepare_start net_mode=$NET_MODE vip="$VIP_ADDR" port="$VIP_PORT"
  preprep_start_ms="$(ms)"
  vip_del_dst || true
  nat_clear_dst || true
  src_forward_clear || true
  emit_event dst_preprepare_done dur_ms=$(( $(ms) - preprep_start_ms ))
fi
...
case "$POSTCOPY_IMAGE_TRANSFER_MODE" in
  shared_nfs)
    ...
    if [ "$(bool_to_int "$POSTCOPY_PREP_DST_BEFORE_CHECKPOINT")" = "1" ]; then
      emit_event transfer_shared_nfs_prepare_skipped reason=preprepared
    else
...
emit_event transfer_verify_start mode="$POSTCOPY_TRANSFER_VERIFY_MODE" image_transfer_mode="$POSTCOPY_IMAGE_TRANSFER_MODE"
verify_start_ms="$(ms)"
SRC_CNT=0
SRC_SUM=0
DST_CNT=0
DST_SUM=0
case "$POSTCOPY_TRANSFER_VERIFY_MODE" in
  none)
    ;;
  fast)
    if [ "$POSTCOPY_IMAGE_TRANSFER_MODE" = "shared_nfs" ]; then
      $SSH "[ -s '$IMAGES_BASE_DST/final/inventory.img' ]" || fail "inventory.img am Ziel fehlt nach Shared-NFS-Check."
    else
  ...
  full)
    ...
    ;;
  *)
    fail "Unbekannter POSTCOPY_TRANSFER_VERIFY_MODE='$POSTCOPY_TRANSFER_VERIFY_MODE' (erlaubt: none, fast, full)"
    ;;
esac
emit_event transfer_verify_done mode="$POSTCOPY_TRANSFER_VERIFY_MODE" dur_ms=$(( $(ms) - verify_start_ms ))
...
if [ "$(bool_to_int "$POSTCOPY_PREP_DST_BEFORE_CHECKPOINT")" = "1" ]; then
  emit_event vip_prepare_prepared_before_checkpoint active=1
else
  vip_del_dst || true
  nat_clear_dst || true
  src_forward_clear || true
...
run "$SSH \"sudo mkdir -p '$DST_WORK_DIR' '$DST_RUNTIME_DIR' && sudo bash -c 'nohup criu lazy-pages \
...
restore_gate_min_ms="$(nonnegative_int_or_default "$POSTCOPY_RESTORE_GATE_MIN_AFTER_CHECKPOINT_MS" 4500)"
restore_gate_start_ms="$(ms)"
restore_gate_elapsed_ms=$((restore_gate_start_ms - t_dump_start))
restore_gate_wait_ms=$((restore_gate_min_ms - restore_gate_elapsed_ms))
emit_event restore_gate_start min_after_checkpoint_ms="$restore_gate_min_ms" elapsed_ms="$restore_gate_elapsed_ms"
if [ "$restore_gate_wait_ms" -gt 0 ]; then
  emit_event restore_gate_wait wait_ms="$restore_gate_wait_ms"
  sleep "$(ms_to_sleep_s "$restore_gate_wait_ms")"
else
  emit_event restore_gate_no_wait elapsed_ms="$restore_gate_elapsed_ms"
fi
emit_event restore_gate_done elapsed_ms=$(( $(ms) - t_dump_start ))
...
run "$SSH \"sudo runc $RUNC_ROOT delete -f '$NAME' 2>/dev/null || true; sudo runc $RUNC_ROOT restore \
  $RUNC_RUN_FLAGS \
...
echo "transfer_verify_mode=$POSTCOPY_TRANSFER_VERIFY_MODE"
echo "lazy_debug_checks=$POSTCOPY_LAZY_DEBUG_CHECKS"
echo "prep_dst_before_checkpoint=$POSTCOPY_PREP_DST_BEFORE_CHECKPOINT"
echo "remote_events=$POSTCOPY_REMOTE_EVENTS"
echo "restore_gate_min_after_checkpoint_ms=$POSTCOPY_RESTORE_GATE_MIN_AFTER_CHECKPOINT_MS"
