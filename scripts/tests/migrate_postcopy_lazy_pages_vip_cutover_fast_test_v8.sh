...
if [ "$lazy_socket_wait_ms" -gt 0 ]; then
  ...
  if run "$SSH \"for i in \$(seq 1 $lazy_socket_attempts); do test -S '$DST_WORK_DIR/lazy-pages.socket' && exit 0; sleep $lazy_socket_interval_s; done; test -S '$DST_WORK_DIR/lazy-pages.socket'\""; then
