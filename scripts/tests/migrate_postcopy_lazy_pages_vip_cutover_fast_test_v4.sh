...
POSTCOPY_IMAGE_READY_MODE="${POSTCOPY_IMAGE_READY_MODE:-stable_core}"
POSTCOPY_IMAGE_READY_MIN_FILES="${POSTCOPY_IMAGE_READY_MIN_FILES:-40}"
POSTCOPY_IMAGE_READY_STABLE_ROUNDS="${POSTCOPY_IMAGE_READY_STABLE_ROUNDS:-2}"
POSTCOPY_IMAGE_READY_INTERVAL_MS="${POSTCOPY_IMAGE_READY_INTERVAL_MS:-200}"
POSTCOPY_IMAGE_READY_TIMEOUT_MS="${POSTCOPY_IMAGE_READY_TIMEOUT_MS:-6000}"
...
emit_event_remote(){
  if [ "${POSTCOPY_REMOTE_EVENTS:-0}" != "1" ]; then
    return 0
  fi
...
wait_images_ready_on_dst(){
  local dst="$1"
  local timeout_ms interval_ms stable_needed min_files mode start now dur_ms
  local stable prev_count prev_size out count size
  mode="${POSTCOPY_IMAGE_READY_MODE:-stable_core}"
  timeout_ms="$(nonnegative_int_or_default "$POSTCOPY_IMAGE_READY_TIMEOUT_MS" 6000)"
  interval_ms="$(nonnegative_int_or_default "$POSTCOPY_IMAGE_READY_INTERVAL_MS" 200)"
  stable_needed="$(positive_int_or_default "$POSTCOPY_IMAGE_READY_STABLE_ROUNDS" 2)"
  min_files="$(positive_int_or_default "$POSTCOPY_IMAGE_READY_MIN_FILES" 40)"
  emit_event image_ready_wait_start mode="$mode" min_files="$min_files" stable_rounds="$stable_needed" \
  ...
  while true; do
    ...
    dur_ms=$((now - start))
    if [ "$dur_ms" -ge "$timeout_ms" ]; then
      emit_event image_ready_timeout dur_ms="$dur_ms" stable="$stable" count="${count:-0}" size="${size:-0}"
      return 1
    fi
    case "$mode" in
      none)
        emit_event image_ready_skipped mode=none
        return 0
        ;;
      inventory)
        if $SSH "[ -s '$dst/final/inventory.img' ]"; then
          emit_event image_ready_ok mode=inventory dur_ms="$dur_ms"
          return 0
        fi
        ;;
      stable_core)
        out="$($SSH "if [ -s '$dst/final/inventory.img' ] && [ -s '$dst/final/pstree.img' ] && [ -s '$dst/final/files.img' ] && ls '$dst/final'/pagemap-*.img >/dev/null 2>&1; then count=\\\$(find '$dst/final' -maxdepth 1 -type f 2>/dev/null | wc -l); size=\\\$(du -sb '$dst/final' 2>/dev/null | awk '{print \\\$1}'); printf '%s %s' \\\"\\\$count\\\" \\\"\\\$size\\\"; else printf '0 0'; fi" 2>/dev/null)" || out="0 0"
        count="${out%% *}"
        size="${out##* }"
        if ! [[ "$count" =~ ^[0-9]+$ ]]; then count=0; fi
        if ! [[ "$size" =~ ^[0-9]+$ ]]; then size=0; fi
        if [ "$count" -ge "$min_files" ] && [ "$count" = "$prev_count" ] && [ "$size" = "$prev_size" ]; then
          stable=$((stable + 1))
        elif [ "$count" -ge "$min_files" ]; then
          stable=1
        else
          stable=0
        fi
        emit_event image_ready_probe mode="$mode" dur_ms="$dur_ms" count="$count" size="$size" stable="$stable"
        if [ "$stable" -ge "$stable_needed" ]; then
          emit_event image_ready_ok mode="$mode" dur_ms="$dur_ms" count="$count" size="$size" stable="$stable"
          return 0
        fi
        prev_count="$count"
        prev_size="$size"
        ;;
      *)
        fail "Unbekannter POSTCOPY_IMAGE_READY_MODE='$mode' (erlaubt: none, inventory, stable_core)"
        ;;
    esac
    sleep "$(ms_to_sleep_s "$interval_ms")"
  done
}
...
emit_event vip_cutover_config \
  ...
  image_ready_mode="$POSTCOPY_IMAGE_READY_MODE" image_ready_min_files="$POSTCOPY_IMAGE_READY_MIN_FILES" \
  image_ready_stable_rounds="$POSTCOPY_IMAGE_READY_STABLE_ROUNDS" image_ready_interval_ms="$POSTCOPY_IMAGE_READY_INTERVAL_MS" \
  image_ready_timeout_ms="$POSTCOPY_IMAGE_READY_TIMEOUT_MS" lazy_debug_checks="$POSTCOPY_LAZY_DEBUG_CHECKS" \
...
wait_images_ready_on_dst "$IMAGES_BASE_DST" || fail "Images am Ziel wurden nicht rechtzeitig bereit."
...
echo "image_ready_mode=$POSTCOPY_IMAGE_READY_MODE"
echo "image_ready_min_files=$POSTCOPY_IMAGE_READY_MIN_FILES"
echo "image_ready_stable_rounds=$POSTCOPY_IMAGE_READY_STABLE_ROUNDS"
echo "image_ready_interval_ms=$POSTCOPY_IMAGE_READY_INTERVAL_MS"
echo "image_ready_timeout_ms=$POSTCOPY_IMAGE_READY_TIMEOUT_MS"
