...
# Conservative v23 minimal-path variant of
# scripts/migrate_postcopy_lazy_pages_vip_cutover.sh.
# Compared with v22, CRIU images are streamed uncompressed from the source to
# local destination storage with tar-over-SSH. Source forwarding and the
# make-before-break cutover remain unchanged.
...
SSH="ssh -n -o BatchMode=yes -o Compression=no -o ConnectTimeout=5 -o StrictHostKeyChecking=accept-new ${DST_USER}@${DST_HOST}"
...
transfer_images_direct_tar(){
  ...
  tar -C "$IMAGES_BASE_SRC" -cf - . | $SSH "sudo rm -rf '$IMAGES_BASE_DST' '$DST_WORK_DIR' '$DST_RUNTIME_DIR' && sudo mkdir -p '$IMAGES_BASE_DST' && sudo tar -C '$IMAGES_BASE_DST' -xpf - && sudo mkdir -p '$DST_WORK_DIR' '$DST_RUNTIME_DIR' && test -s '$IMAGES_BASE_DST/final/inventory.img'"
}
...
case "$RUNC_BUNDLE_DST" in
  "$REMOTE_NFS_ROOT"|"$REMOTE_NFS_ROOT"/*)
    fail "Lokales Bundle-Ziel darf nicht unter dem Destination-NFS liegen: $RUNC_BUNDLE_DST"
    ;;
esac
case "$IMAGES_BASE_DST" in
  "$REMOTE_NFS_ROOT"|"$REMOTE_NFS_ROOT"/*)
    fail "Lokales Image-Ziel darf nicht unter dem Destination-NFS liegen: $IMAGES_BASE_DST"
    ;;
esac
[ "$IMAGES_BASE_DST" != "$IMAGES_BASE_DST_SHARED_SRC" ] || fail "Lokales Image-Ziel darf nicht dem Shared-Image-Pfad entsprechen"
...
if [ "$POSTCOPY_SRC_FORWARD_ENABLED" = "1" ]; then
  [ "$NET_MODE" = "host" ] || fail "Source-Forwarding in v23 unterstützt nur NET_MODE=host"
  [ "$POSTCOPY_SRC_FORWARD_MODE" = "iptables_dnat" ] || fail "Source-Forwarding in v23 unterstützt nur POSTCOPY_SRC_FORWARD_MODE=iptables_dnat"
...
emit_event script_start mode="$MODE" name="$NAME" run_id="$RUN_ID" variant=v23_minimal transfer_mode=direct_tar compression=none src_forward="$POSTCOPY_SRC_FORWARD_ENABLED"
...
case "$POSTCOPY_BUNDLE_PREPARE_MODE" in
  copy)
    $SSH "test -d '$RUNC_BUNDLE_DST_SHARED' && mount | grep -q '$REMOTE_NFS_ROOT'" || fail "Shared runc-Bundle oder Destination-NFS fehlt"
    ;;
  reuse)
...
t_tx_start="$(ms)"
transfer_images_direct_tar || fail "Direkter tar-over-SSH-Transfer fehlgeschlagen"
t_tx_end="$(ms)"
emit_event transfer_done method=direct_tar files=not_collected bytes=not_collected compression=none dur_ms=$((t_tx_end - t_tx_start))
...
emit_event summary mode="$MODE" name="$NAME" cp="$CP_NAME" variant=v23_minimal transfer_mode=direct_tar compression=none images_src="$IMAGES_BASE_SRC" images_dst="$IMAGES_BASE_DST" bundle_dst="$RUNC_BUNDLE_DST" vip="$VIP_ADDR" net_mode="$NET_MODE" src_forward="$POSTCOPY_SRC_FORWARD_ENABLED" vip_cutover_strategy="$vip_cutover_strategy"
...
log "Minimale v23 Post-Copy-Migration + Direct-Tar + Forwarding/VIP-Cutover abgeschlossen."
